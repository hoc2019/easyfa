...
export default function parseAPNG(
  ...
  apng.frames.forEach(frame => {
    ...
    bb.push(preBlob);
    frame.dataParts.forEach((p: Uint8Array) =>
      bb.push(makeChunkBytes("IDAT", p))
    );
    bb.push(postBlob);
 ...
 * @return {Uint8Array}
 */
function makeStringArray(x: string): Uint8Array {
 ...
 * @return {Uint8Array}
 */
var makeChunkBytes = function(type: string, dataBytes: Uint8Array) {
